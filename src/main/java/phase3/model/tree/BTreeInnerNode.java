...
class BTreeInnerNode extends BTreeNode {
	...
	public BTreeInnerNode() {
		this.keys = new SimpleEntity[INNERORDER + 1];
	...
	private void insertAt(int index, SimpleEntity key, BTreeNode leftChild, BTreeNode rightChild) {
	...
	protected void processChildrenTransfer(BTreeNode borrower, BTreeNode lender, int borrowIndex) {
		...
		if (borrowIndex == 0) {
			...
			SimpleEntity upKey = borrower.transferFromSibling(this.getKey(borrowerChildIndex), lender, borrowIndex);
			this.setKey(borrowerChildIndex, upKey);
		} else {
			// borrow a key from left sibling
			SimpleEntity upKey = borrower.transferFromSibling(this.getKey(borrowerChildIndex - 1), lender, borrowIndex);
	...
	protected BTreeNode processChildrenFusion(BTreeNode leftChild, BTreeNode rightChild) {
		...
		while (index < this.getKeyCount() && this.getChild(index) != leftChild)
			++index;
		SimpleEntity sinkKey = this.getKey(index);
